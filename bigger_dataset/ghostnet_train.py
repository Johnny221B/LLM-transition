...
# Dataset parameters
parser.add_argument('--data_dir', metavar='DIR', default='./data/flowers',
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                epoch, model, teacher, loader_train, optimizer, train_loss_fn, kd_loss, args,
...
def train_one_epoch(
        epoch, model, teacher, loader, optimizer, loss_fn, kd_loss, args,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            loss_kd = args.kd_loss_weight * kd_loss(z_s=output, z_t=output_t)
            loss = loss_ori + loss_kd

        if not args.distributed:
            losses_m.update(loss.item(), input.size(0))
            losses_ori_m.update(loss_ori.item(), input.size(0))
            losses_kd_m.update(loss_kd, input.size(0))
